...
{
  int sum = 0;
  int smaller_than_2 = 0;
  int smaller_than_6 = 0;
  char two[] = "[[2]]";
  char six[] = "[[6]]";
  ...
  for(int i=0;i<(sizeof(data)/sizeof(data[0]));i+=2)
  {
    ...
    if(compare_strings(left_p,two) < 0)
      smaller_than_2++;
    if(compare_strings(right_p,two) < 0)
      smaller_than_2++;
    if(compare_strings(left_p,six) < 0)
      smaller_than_6++;
    if(compare_strings(right_p,six) < 0)
      smaller_than_6++;
    
  }

  printf("End with sum %i\n",sum);
  printf("Part2 decoder key %i\n",(smaller_than_2+1)*(smaller_than_6+2)); // add 2 since [[2]] takes another slot.
  return 0;
}
