...
int main(int argc, char* argv[])
{
  ...
  vismap[sy][sx] = 1; // This is visited!
  ...
  while(1)
  {
    while((now_p[now_idx].y)>=0)
    {
      ...
      //Queue all possible
      // Left?
      if(x && ((data[y][x-1] - data[y][x]) <= 1) && !vismap[y][x-1])
      {
        vismap[y][x-1] = 1;
        next_p[next_idx].y = y;
      ...
      // Right?
      if(((x+1)<mx) && ((data[y][x+1] - data[y][x]) <= 1)  && !vismap[y][x+1])
      {
        vismap[y][x+1] = 1;
        next_p[next_idx].y = y;
        next_p[next_idx].x = x+1;
      ...
      // Up?
      if(y && ((data[y-1][x] - data[y][x]) <= 1)  && !vismap[y-1][x])
      {
        vismap[y-1][x] = 1;
      ...
      // Down?
      if(((y+1)<my) && ((data[y+1][x] - data[y][x]) <= 1)  && !vismap[y+1][x])
      {
        vismap[y+1][x] = 1;
        next_p[next_idx].y = y+1;
        next_p[next_idx].x = x;
