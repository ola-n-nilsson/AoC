...
int main(int argc, char* argv[])
{
  ...
  {
    /* printf("Selected: %lu (%i, %i, %i) (%i, %i, %i)\n",sdist[i].dist,data[(sdist[i].p2)] */
    /*                                                ,data[(sdist[i].p2)+1] */
    /*                                                ,data[(sdist[i].p2)+2] */
    /*                                                ,data[(sdist[i].p1)] */
    /*                                                ,data[(sdist[i].p1)+1] */
  ...
  qsort (longest, lidx, sizeof(int), compint);

  printf("Biggest %i %i %i %li\n",longest[0],longest[1],longest[2],(long)longest[0]*longest[1]*longest[2]);
  return 0;
}
